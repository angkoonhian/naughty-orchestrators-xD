...
| Skill | One-liner |
|---|---|
| **`orchestrate`** | Bootstraps **and** operates a multi-tier *agent orchestration* system in any codebase — scan → infer → generate a tailored `CLAUDE.md` hierarchy, then run a dispatch loop with a Devil's-Advocate gate and pre/post-implementation validation gates. |
...
The interesting part: **`orchestrate` can consume a `graphify` graph** to make its impact assessment and
routing *structure-aware* instead of heuristic-only. The two halves of this repo are designed to click together.
...
`da-lead` fans out **in parallel** to **7 universal core critics** — `edge-case`, `security`, `performance`,
`failure-mode`, `consistency`, `tech-debt`, `alternatives` (always proposes a Plan B) — plus any matched
**platform-pack** critics (e.g. `cross-tenant-leak`, `jwt-lifecycle`, `socket-fanout`, `multi-db-consistency`). It
de-dupes findings and returns one verdict: **PROCEED / PROCEED-WITH-CHANGES / RECONSIDER**.
...
│   │                            # + graph_build · graph_bake · blast_radius (+ tests/)
│   ├── assets/                  # templates, 7 core critics, validators, task-agents
│   ├── platform-packs/          # 16 domain critic bundles (pack.yaml + critics)
│   └── references/              # loop-semantics, classification-rules, graph-integration, …
└── graphify/
...
*Two skills, one toolbox. `orchestrate` decides **who** does the work and **how carefully**; `graphify` tells it
**what actually depends on what**. Naughty, but organized. 😈*
